...
subtest 'job setting based retriggering' => sub {
    ...
    my $count_jobs = sub ($task) {
        my @res;
        $minion->jobs({tasks => [$task]})->each(sub ($j) { push @res, $j });
        \@res;
    };
    my $restart_job_count_before = @{$count_jobs->('restart_job')};
    my $finalize_job_count_before = @{$count_jobs->('finalize_job_results')};
    $job->update({state => SCHEDULED, result => NONE});
    $job->done(result => FAILED);
    perform_minion_jobs($minion);
    ...
    my $restart_jobs = $count_jobs->('restart_job');
    my $finalize_jobs = $count_jobs->('finalize_job_results');
