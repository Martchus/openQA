...
is($job_groups->find(1001)->exclusively_kept_asset_size, undef, 'initially no size for exclusively kept assets accumulated');
...
is_deeply(
    find_kept_assets_with_last_jobs, \@expected_last_jobs_no_removal,
...
is_deeply(
    find_kept_assets_with_last_jobs, \@expected_last_jobs_no_removal,
    'last jobs have not been altered'
);

is($job_groups->find(1001)->exclusively_kept_asset_size,
   60 * $gib, 'kept assets for group 1001 accumulated (30 GiB per asset)');
# FIXME: I would have expected that sum for 1001 is 90 for 3 * 30 GiB
is($job_groups->find(1002)->exclusively_kept_asset_size,
