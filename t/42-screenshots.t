...
# pretend the screenshots have no link_count set (possible when migration is still ongoing)
# and add the screenshot again to check whether an undefined link_count won't be a problem
$screenshots->search->update({link_count => undef});
OpenQA::Schema::Result::ScreenshotLinks::populate_images_to_job($schema, [qw(foo)], 99927);
@screenshots     = $screenshots->search({id => {-in => \@screenshot_ids}})->search({}, {order_by => 'id'});
@screenshot_data = map { {filename => $_->filename, link_count => $_->link_count} } @screenshots;
is_deeply(
    \@screenshot_data,
    [{filename => 'foo', link_count => 1}, {filename => 'bar', link_count => undef},],
    'link_count for foo set from undef to 1'
) or diag explain \@screenshot_data;

# delete job during migration
is($jobs->find(99926)->delete,
    0, 'job deletion fails when migration has not been completed (link_count of bar is still undef here)');
