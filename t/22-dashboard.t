...
is_deeply(\@h4, [map { 'Build' . $_ } qw(0048@0815 0048 0092)], 'builds on parent-level shown, sorted first by version');
...
sub check_test_parent {
    ...
    $t->element_count_is('#review-also-softfailed-' . $test_parent->id . '-Factory-0048@0815',
    ...
    @h4 = $get->tx->res->dom->find('div#group' . $test_parent->id . '_build13_1_0091 h4 a')->map('text')->each;
    is_deeply(\@h4, ['opensuse', 'opensuse test'], 'both child groups shown under common build');
    @progress_bars
      = $get->tx->res->dom->find('div#group' . $test_parent->id . '_build13_1_0091 .progress')->map('attr', 'title')->each;
    is_deeply(
        \@progress_bars,
    ...
    my @urls = $get->tx->res->dom->find('div#group' . $test_parent->id . '_build13_1_0091 h4 a')->map('attr', 'href')->each;
    is_deeply(
        \@urls,
        [
...
my $review_build_id = '-Factory_0048@0815';
