...
Job dependencies now allow inter-machine dependency that make possible to create job dependencies between tests that are scheduled for the different machine.

To use it, simply append machine for each dependent test suite with colon separated.
If machine isn't explicitly defined, the variable %MACHINE% of the current job is used for dependent test suite.

E.g. +START_AFTER_TEST="kde:64bit-1G,dhcp-server:64bit-8G"+
or
E.g. +PARALLEL_WITH="web-server:ipmi-fly,dhcp-server:ipmi-bee",http-server+

Then in job templates, add test suite(s) and all of its dependent test suite(s), keep in mind to place the machine that have explicitly defined in variable for each dependent test suite.

Let's see some examples to have better understand.

===== Examples:

Correct dependency and machine placed
----
...
----
Best match with correct dependency and machine placed
----
...
A:ppc <- B:ppc
----
Wrong dependency or wrong machine placed
----
If test suite A placed on machine 64bit-8G,
then test suite B placed on machine 64bit-1G with variable START_AFTER_TEST=A
openQA won't create job dependency and give an error message unless you have explicitly define variable as START_AFTER_TEST=A:64bit-8G
...
openQA will create dependency for this case:
            A:ppc
              ^
           /  |  \
         /    |    \
...
But also give errors to inform users that test suite A on machines 64bit and s390x are not necessary.
----
Dependency on same machine
----
If variable START_AFTER_TEST or PARALLEL_WITH with *only* name(s) of test suite(s).
START_AFTER_TEST=A,B or PARALLEL_WITH=A,B
openQA will create job dependencies that are scheduled on the same machine if all test suites placed on the same machine.
----
