...
sub {
    ...
    my $audit_events       = $schema->resultset('AuditEvents')->search(
        {event    => 'iso_create'},
        {order_by => {-asc => 'me.id'}},
    );
    ...
    while (my $event = $audit_events->next) {
        ...
        try {
            ...
            OpenQA::Utils::log_warning("Unable to read settings from 'iso_create' audit event with ID $event_id. Skipping its migration.");
        };
