...
sub _prepare_job_results ($self, $jobs, $job_ids, $limit) {
    my %archs;
    my %results;
    my @job_ids;
    my $aggregated = {
    ...
    # get all jobs; there can be one more job than the configured limit which means the limit was exceeded
    my @jobs = $jobs->all;
    pop @jobs if my $exceeded_limit = @jobs >= $limit ? $limit : undef;
    ...
    return ($exceeded_limit, \%archs, \%results, $aggregated);
}
...
sub _every_non_empty_param ($self, $param_key) {
    [grep { $_ ne '' } @{$self->every_param($param_key)}]
}

sub _compute_overview_filtering_params ($self) {
    my $states = $self->_every_non_empty_param('state');
    my $results = $self->_every_non_empty_param('result');
    my $archs = $self->_every_non_empty_param('arch');
    my $machines = $self->_every_non_empty_param('machine');
    my $failed_modules = $self->_every_non_empty_param('failed_modules');
    my %filters;
...
sub overview ($self) {
    ...
    $search_args->{filters} = $self->_compute_overview_filtering_params;
    ...
    my $jobs = $jobs_rs->search({id => {-in => $latest_job_ids}});
    my ($exceeded_limit, $archs, $results, $aggregated)
      = $self->_prepare_job_results($jobs, $latest_job_ids, $search_args->{limit} - 1);
