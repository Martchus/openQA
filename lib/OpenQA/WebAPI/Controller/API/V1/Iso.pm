...
sub _sort_dep {
    ...
    do {
        ...
        for my $job (@$list) {
            ...
            foreach my $a (@after) {
                $c += $count{join(':', @$a)} if defined $count{join(':', @$a)};
            }
...
sub job_create_dependencies {
        ...
        ['PARALLEL_WITH',    OpenQA::Schema::Result::JobDependencies::PARALLEL])
    {
        ...
        for my $testsuite (_parse_dep_variable($settings->{$depname}, $settings)) {
            ...
            for my $m (keys %{$testsuite_mapping->{$test}}) {
                if (!exists $cluster_parents->{$test . ':' . $m}) {
                    $cluster_parents->{$test . ':' . $m} = $testsuite_mapping->{$test}->{$m};
                }
            }
...
sub schedule_iso {
    ...
    my $coderef = sub {
        ...
        # log wrong parents
        for my $k (keys %cluster_parents) {
            if ($cluster_parents{$k} ne 'depended') {
                my $error_msg = "$k has no child, check its machine placed or dependency setting typos";
                OpenQA::Utils::log_warning($error_msg);
                push(
                    @failed_job_info,
                    {
                        job_id         => $cluster_parents{$k},
