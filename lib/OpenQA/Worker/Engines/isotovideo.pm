...
sub _poll_cache_service ($job, $cache_client, $request, $status_ref, $delay, $callback) {
    my $io_loop = Mojo::IOLoop->singleton;
    return $io_loop->timer(
        $delay => sub {
            _poll_cache_service($callback, $job, $cache_client, $request, $status_ref, $delay);
        }) unless $$status_ref->is_processed;

    return $callback->({error => 'Status updates interrupted'}) unless $job->post_setup_status;
    return $callback->({error => 'Job has been cancelled'}) if $job->is_stopped_or_stopping;
    return $callback->({error => $$status_ref->error})      if $$status_ref->has_error;
...
sub cache_assets ($cache_client, $job, $vars, $assets_to_cache, $assetkeys, $webui_host, $pooldir, $callback) {
    ...
    return _poll_cache_service(
        ...
        sub ($error) {
            ...
            return cache_assets($job, $vars, $assets_to_cache, $assetkeys, $webui_host, $pooldir, $callback);
        });
}
