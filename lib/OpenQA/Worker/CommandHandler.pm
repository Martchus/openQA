...
sub _can_grab_job {
    ...
    # determine the jobs currently being executed/pending
    my %current_job_ids = map { $_ => 1 } @{$worker->pending_job_ids};
    $current_job_ids{$current_job->id} = 1 if defined $current_job;
    ...
    if ($current_webui_host ne $webui_host) {
