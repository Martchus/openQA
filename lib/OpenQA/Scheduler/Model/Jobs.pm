    ...
    for my $allocated (values %$allocated_jobs) {
        ...
        for my $job (@jobs) {
            ...
            if ($job->state ne SCHEDULED) {
                log_debug("Job $job_id no longer scheduled, skipping");
                $skip = 1;
                last;
            }
        }
...
sub _serialize_directly_chained_job_sub_sequence {
    ...
    for my $current_job_id (@$child_job_ids) {
        die "cycle at $current_job_id" if $visited->{$current_job_id}++;
        my $sub_sequence
...
sub _assign_multiple_jobs_to_worker {
    ...
    for my $job (@$jobs) {
        my $job_id   = $job->id;
        my $job_data = $job->prepare_for_work($worker, $job_id ne $first_job);
