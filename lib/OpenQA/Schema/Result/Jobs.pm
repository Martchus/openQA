...
sub _compute_asset_names_considering_parent_jobs {
    ...
    return [$asset_name, map { sprintf("%08d-%s", $parent_job_ids, $asset_name) } @$parent_job_ids];
}
...
sub missing_assets {
    ...
    my @assets_query
      = map { {type => $_->{type}, name => {-in => _compute_asset_names_considering_parent_jobs($parent_job_ids, $_->{name})}} }
      @relevant_assets;
...
sub _parent_job_ids {
    my ($self)    = @_;
    my %condition = (dependency => OpenQA::JobDependencies::Constants::CHAINED);
